...
async function ensureProcessedFolder(client: ImapFlow, folder: string): Promise<void> {
...
export async function runEmailImport(): Promise<ImportRunResult> {
  ...
  const result: ImportRunResult = { processed: 0, imported: 0, skipped: 0, errors: 0 };
  ...
  try {
    ...
    await ensureProcessedFolder(client, processedFolder);
